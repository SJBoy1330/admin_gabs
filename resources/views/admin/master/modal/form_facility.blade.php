...
<div class="d-flex flex-column me-n7 pe-7" id="#">
    ...
    <div class="fv-row mb-7 d-flex justify-content-center align-items-center flex-column">
        <!--begin::Label-->
        <label class="d-block fw-semibold fs-6 mb-5 required">Icon</label>
        <!--end::Label-->
        <!--begin::Image input-->
        <div class="image-input background-partisi" data-kt-image-input="true" style="background-image: url('<?= image_check('notfound.jpg','default') ?>')">
            ...
            <div id="display_image" class="image-input-wrapper w-125px h-125px background-partisi" style="background-image: url('<?= (isset($result->image)) ? image_check($result->image,'facility') : image_check('notfound.jpg','default') ?>')"></div>
    ...
    <!--end::Input group-->
    <input type="hidden" name="id_facility" value="{{ (isset($result->id_facility)) ? $result->id_facility : 0 }}">
    ...
    <div class="fv-row mb-7">
        <!--begin::Label-->
        <label class="required fw-semibold fs-6 mb-2">Nama Fasilitas <span class="text-primary">({{ $row->name }})</span></label>
        <!--end::Label-->
        <!--begin::Input-->
        <input type="text" name="name_{{ $row->id_language }}" value="{{ optional($result?->details->firstWhere('id_language', $row->id_language))->name ?? '' }}" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="Masukkan Nama Fasilitas ({{ $row->name }})" autocomplete="off" />
        <!--end::Input-->
    </div>
    <!--end::Input group-->
    @endforeach
...
<div class="text-center pt-15">
    <button type="button" id="submit_facility" data-loader="big" onclick="submit_form(this,'#form_facility')" class="btn btn-primary">
