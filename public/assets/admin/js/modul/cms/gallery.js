...
function action_data(id = null) {
    let form = document.getElementById('form_gallery');
    let title = $('#title_modal').data('title').split('|');

    if (id == null) {
        $('#title_modal').text(title[1]);
        form.setAttribute('action', BASE_URL + '/cms/gallery/insert');
    } else {
        $('#title_modal').text(title[0]);
        form.setAttribute('action', BASE_URL + '/cms/gallery/update');
    }

    $.ajax({
        url: BASE_URL + '/modal_gallery',
        method: 'POST',
        data: { _token: csrf_token, id: id },
        cache: false,
        beforeSend: function () {
            $('#display_form_gallery').html(div_loading);
        },
        success: function (msg) {
            $('#display_form_gallery').html(msg);
