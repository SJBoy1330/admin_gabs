...
document.addEventListener('DOMContentLoaded', function () {
    // Init DataTable
    articleTable = initGlobalDatatable('#table_article', function () {
    ...
    document.querySelectorAll('.table-filter').forEach(el => {
        el.addEventListener('change', function () {
            if (articleTable) articleTable.ajax.reload();
        });
    });
});
...
function action_data(id = null) {
    let form = document.getElementById('form_article');
    let title = $('#title_modal').data('title').split('|');

    if (id == null) {
        $('#title_modal').text(title[1]);
        form.setAttribute('action', BASE_URL + '/cms/article/insert');
    } else {
        $('#title_modal').text(title[0]);
        form.setAttribute('action', BASE_URL + '/cms/article/update');
    }

    $.ajax({
        url: BASE_URL + '/modal_article',
        method: 'POST',
        data: { _token: csrf_token, id: id },
        cache: false,
        beforeSend: function () {
            $('#display_form_article').html(div_loading);
        },
        success: function (msg) {
            $('#display_form_article').html(msg);
